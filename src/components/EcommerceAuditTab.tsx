...
import { Globe, Search, AlertOctagon, CheckCircle2, ArrowRight, ExternalLink } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm">
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 text-indigo-800 border border-indigo-200 text-xs font-bold mb-3">
        ...
        <form onSubmit={handleCustomAudit} className="mt-5 flex flex-wrap sm:flex-nowrap gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-300 text-slate-900 text-xs pl-10 pr-4 py-3 rounded-xl focus:outline-none focus:border-indigo-600 font-mono"
            />
          </div>
          <button
            type="submit"
            disabled={isAuditing}
            className="bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold px-5 py-3 rounded-xl shadow-md transition-all flex items-center justify-center gap-2 shrink-0 cursor-pointer disabled:opacity-50"
          >
          ...
          {ecomSamples.map((sample, idx) => {
            ...
            return (
              <div
                ...
                className="bg-slate-50 border border-slate-200 hover:border-indigo-400 p-4 rounded-xl cursor-pointer transition-all hover:bg-white flex flex-col justify-between group shadow-2xs"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-white text-indigo-900 border border-slate-200">
                      {sample.platform}
                    </span>
                    <span
                  ...
                  <h4 className="text-xs font-black text-slate-900 group-hover:text-indigo-600 transition-colors mb-1.5 line-clamp-2">
                    {sample.title}
                  </h4>
                ...
                <div className="mt-3 pt-2.5 border-t border-slate-200 flex items-center justify-between text-[11px] text-indigo-700 font-bold">
