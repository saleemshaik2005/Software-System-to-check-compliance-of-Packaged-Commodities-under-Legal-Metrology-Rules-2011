...
export const OfficerAnalyticsDashboard: React.FC<OfficerAnalyticsDashboardProps> = ({
  ...
  // Fourth Schedule MPE logic
  const getMpeValue = (q: number) => {
    if (q <= 50) return { mpeText: '9%', mpeVal: q * 0.09 };
    if (q <= 100) return { mpeText: '4.5 g/ml', mpeVal: 4.5 };
    if (q <= 200) return { mpeText: '4.5%', mpeVal: q * 0.045 };
    if (q <= 300) return { mpeText: '9 g/ml', mpeVal: 9 };
    if (q <= 500) return { mpeText: '3%', mpeVal: q * 0.03 };
    if (q <= 1000) return { mpeText: '15 g/ml', mpeVal: 15 };
    if (q <= 10000) return { mpeText: '1.5%', mpeVal: q * 0.015 };
    if (q <= 15000) return { mpeText: '150 g/ml', mpeVal: 150 };
    return { mpeText: '1%', mpeVal: q * 0.01 };
  };

  const sampling = getSamplingPlan(lotSize);
  const mpe = getMpeValue(declaredQty);
  ...
  const handleDelete = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (window.confirm('Delete this inspection record from the local dossier?')) {
      deleteScanReport(id);
...
Inspecting Officer: ${noticeReport.inspectorName || 'Inspector R. K. Verma'} (Badge: ${noticeReport.inspectorBadgeNumber || 'LM-ND-4092'})
...
Issued By:
${noticeReport.inspectorName || 'Inspector R. K. Verma'}
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300">
                  Rule 24 Official Formulas
                </span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Calculate required statistical sample size $n$, acceptable defect limit $c$, and Maximum Permissible Error (MPE) for batch raids
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
            <span>{isCalcOpen ? 'Hide Calculator' : 'Open Field Tool'}</span>
            {isCalcOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
          </div>
        </button>

        {isCalcOpen && (
          <div className="p-5 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/40 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 p-2 rounded-xl text-xs font-bold text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-600"
                />
                <span className="text-[10px] text-slate-400 mt-1 block">
                  Fifth Schedule Table 1 tiered ranges
                </span>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <select
                    ...
                    className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 p-2 rounded-xl text-xs font-bold text-slate-900 dark:text-slate-100"
                  >
                ...
                <span className="text-[10px] text-slate-400 mt-1 block">
                  Fourth Schedule MPE threshold
                </span>
              </div>

              <div className="sm:col-span-1 flex flex-col justify-end">
                <div className="p-3 bg-blue-50 dark:bg-blue-950/60 border border-blue-200 dark:border-blue-800 rounded-xl text-xs">
                  <div className="font-bold text-blue-900 dark:text-blue-300">Statistical Test Condition:</div>
                  <div className="text-[11px] font-mono text-blue-700 dark:text-blue-400 mt-0.5">
                    Avg Qty ≥ Qn - ({sampling.tFactor} × s / √{sampling.sampleSize})
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
              <div className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <span className="text-xl font-black text-[#0A3663] dark:text-blue-400 mt-0.5 block">
                  {sampling.sampleSize} Packs
                </span>
                <span className="text-[10px] text-slate-500">Per Rule 24 Table 1</span>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <span className="text-[10px] text-slate-500">Exceeding MPE allowed</span>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <span className="text-xl font-black text-emerald-600 mt-0.5 block">
                  {mpe.mpeText} ({mpe.mpeVal.toFixed(2)} {qtyUnit})
                </span>
                <span className="text-[10px] text-slate-500">Fourth Schedule Table</span>
              </div>

              <div className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                <span className="text-[10px] uppercase font-bold text-slate-400 block">Single Unit Defective Boundary</span>
                <span className="text-xl font-black text-red-600 mt-0.5 block">
                  &lt; {minPermittedDefectiveBoundary.toFixed(2)} {qtyUnit}
                </span>
                <span className="text-[10px] text-slate-500">Classified as defective pack</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {repeatOffenders.map((offender, idx) => (
              <div key={idx} className="bg-white dark:bg-slate-900 border border-red-200 dark:border-red-800 p-3 rounded-xl">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-black text-slate-900 dark:text-slate-100 truncate">{offender.name}</span>
                ...
                <div className="text-[11px] text-slate-500 mt-1">
                  Last product: <span className="text-slate-700 dark:text-slate-300 font-semibold">{offender.lastViolation}</span>
                </div>
                <div className="text-[10px] font-mono text-red-600 dark:text-red-400 font-bold mt-1">
                  Mandatory Action: Section 36(2) Notice
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-xs transition-colors">
        <h3 className="text-sm font-black text-slate-900 dark:text-slate-100 mb-3 flex items-center gap-2">
          <BarChart3 className="w-4 h-4 text-[#0A3663] dark:text-blue-400" />
          <span>Category Risk Heatmap (LMPC Enforcement Hotspots)</span>
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
            <div className="flex justify-between items-center mb-1">
              <span className="font-bold text-slate-800 dark:text-slate-200">Edible Oils</span>
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
            <div className="flex justify-between items-center mb-1">
              <span className="font-bold text-slate-800 dark:text-slate-200">Biscuits & Bakery</span>
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
            <div className="flex justify-between items-center mb-1">
              <span className="font-bold text-slate-800 dark:text-slate-200">Dairy & Milk</span>
              <span className="text-[10px] text-amber-600 font-bold">Medium Risk</span>
            </div>
            <div className="w-full bg-slate-200 dark:bg-slate-700 h-1.5 rounded-full overflow-hidden mb-1">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
            <div className="flex justify-between items-center mb-1">
              <span className="font-bold text-slate-800 dark:text-slate-200">Foodgrains / Atta</span>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xs transition-colors">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-slate-100 text-xs pl-8 pr-3 py-1.5 rounded-xl w-48 sm:w-64 focus:outline-none focus:border-[#0A3663]"
              />
            </div>
            ...
            <select
              ...
              className="bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-slate-100 text-xs px-3 py-1.5 rounded-xl focus:outline-none focus:border-[#0A3663]"
            >
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-800">
          <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
            <thead className="bg-slate-50 dark:bg-slate-800/80 text-[11px] uppercase font-bold text-slate-600 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filteredReports.map((report) => {
                ...
                return (
                  ...
                  >
                    <td className="py-3 px-4 font-mono font-bold text-[#0A3663] dark:text-blue-400">
                      {report.id}
                    ...
                    <td className="py-3 px-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <button
                          ...
                          className="p-1.5 bg-slate-100 hover:bg-emerald-600 hover:text-white dark:bg-slate-800 dark:hover:bg-emerald-600 rounded-lg text-slate-600 dark:text-slate-300 transition-colors cursor-pointer"
                        ...
                        <button
                          ...
                          className="p-1.5 bg-slate-100 hover:bg-[#0A3663] hover:text-white dark:bg-slate-800 dark:hover:bg-blue-600 rounded-lg text-slate-600 dark:text-slate-300 transition-colors cursor-pointer"
                        ...
                        <button
                          ...
                          className="p-1.5 bg-slate-100 hover:bg-red-600 hover:text-white dark:bg-slate-800 dark:hover:bg-red-600 rounded-lg text-slate-500 transition-colors cursor-pointer"
                          title="Delete record"
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-3xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between border-b border-slate-800">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 font-mono">Case: {noticeReport.id} • {noticeReport.productInfo.productName}</p>
                </div>
              </div>
              <button
              ...
              <button
                ...
                className={`px-3 py-1.5 text-xs font-bold rounded-t-xl transition-all cursor-pointer ${
                  noticeType === 'SHOW_CAUSE'
                    ? 'bg-white dark:bg-slate-900 text-red-600 dark:text-red-400 border-t border-x border-slate-200 dark:border-slate-800'
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-100 dark:bg-slate-950">
              <pre className="whitespace-pre-wrap font-mono text-xs bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 leading-relaxed shadow-inner">
                {generateStatutoryNoticeText()}
              </pre>
            </div>

            {/* Notice Footer Actions */}
            <div className="px-6 py-3.5 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-xs font-bold text-slate-700 dark:text-slate-300 transition-colors cursor-pointer"
                >
