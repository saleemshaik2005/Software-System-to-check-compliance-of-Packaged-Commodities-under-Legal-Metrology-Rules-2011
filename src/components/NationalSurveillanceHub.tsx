  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-xs flex flex-wrap items-center justify-between gap-4 transition-colors">
        ...
        <div className="flex items-center gap-2">
          <div className="text-right hidden sm:block">
            ...
            <span className="text-xs font-black text-slate-900 dark:text-slate-100">Dr. S. K. Rastogi</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xs transition-colors space-y-4">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800 text-xs">
            {stateData.map((item) => (
              <div key={item.state} className="py-2.5 flex items-center justify-between">
                <div>
                  <span className="font-bold text-slate-900 dark:text-slate-100 block">{item.state}</span>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-[11px] font-mono font-bold text-slate-700 dark:text-slate-300 min-w-[70px] text-right">
                    {item.fine}
                  </span>
                </div>
              </div>
        ...
        <div className="lg:col-span-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xs transition-colors space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
            <div className="flex items-center gap-2">
          ...
          <div className="space-y-3 text-xs">
            {darkStores.map((ds) => (
              <div key={ds.name} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="font-black text-slate-900 dark:text-slate-100">{ds.name}</span>
                  <div className="flex items-center gap-2">
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-800 dark:text-slate-200 text-xs">
              {repeatOffenders.map((ro) => (
                <tr key={ro.brand}>
                  <td className="p-3 font-bold text-slate-900 dark:text-slate-100">{ro.brand}</td>
                  <td className="p-3">
                  ...
                  <td className="p-3 text-slate-600 dark:text-slate-400 text-[11px]">{ro.rule}</td>
