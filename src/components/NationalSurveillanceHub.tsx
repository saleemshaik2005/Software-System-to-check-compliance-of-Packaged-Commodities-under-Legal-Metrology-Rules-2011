...
export const NationalSurveillanceHub: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      {/* Header Banner */}
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl p-6 shadow-xs flex flex-wrap items-center justify-between gap-4 transition-colors">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] bg-cyan-100 dark:bg-cyan-950 text-cyan-900 dark:text-cyan-300 px-2 py-0.5 rounded-full font-bold">
                Live Field Feed
              </span>
            </div>
            <h1 className="text-xl font-black text-slate-900 dark:text-zinc-100 tracking-tight mt-0.5">
        ...
        <div className="flex items-center gap-2">
          <div className="text-right hidden sm:block">
            ...
            <span className="text-xs font-black text-slate-900 dark:text-zinc-100">National Surveillance Director</span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-5 rounded-2xl shadow-xs transition-colors">
          ...
          <div className="text-2xl font-black text-[#0A3663] dark:text-blue-400 mt-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 shadow-xs transition-colors space-y-4">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800 text-xs">
            {stateData.map((item) => (
              <div key={item.state} className="py-2.5 flex items-center justify-between">
                <div>
                  <span className="font-bold text-slate-900 dark:text-zinc-100 block">{item.state}</span>
                  <span className="text-[10px] text-slate-500">{item.raids.toLocaleString()} inspections • {item.seizures} seizures</span>
                </div>

                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <span className="font-mono font-bold text-emerald-600 dark:text-emerald-400 block">{item.compliance}%</span>
                  ...
                  <span className="text-[11px] font-mono font-bold text-slate-700 dark:text-zinc-300 min-w-[70px] text-right">
                    {item.fine}
                  </span>
                </div>
              </div>
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 shadow-xs transition-colors space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-zinc-800 pb-3">
            <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold bg-indigo-50 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300 px-2 py-0.5 rounded-full">
              LMPC Rule 6(10)
            </span>
          </div>
          ...
          <div className="space-y-3 text-xs">
            {darkStores.map((ds) => (
              <div key={ds.name} className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="font-black text-slate-900 dark:text-zinc-100">{ds.name}</span>
                  <div className="flex items-center gap-2">
                    <span className="font-mono font-bold text-[#0A3663] dark:text-blue-400">{ds.compliance}%</span>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 shadow-xs transition-colors space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-zinc-800 pb-3">
          <div className="flex items-center gap-2">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 dark:bg-zinc-950 text-slate-500 text-[11px] border-b border-slate-200 dark:border-zinc-800">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-800 dark:text-zinc-200 text-xs">
              {repeatOffenders.map((ro) => (
                <tr key={ro.brand}>
                  <td className="p-3 font-bold text-slate-900 dark:text-zinc-100">{ro.brand}</td>
                  <td className="p-3">
                  ...
                  <td className="p-3 text-slate-600 dark:text-zinc-400 text-[11px]">{ro.rule}</td>
                  <td className="p-3 font-mono font-bold text-red-600">{ro.penalty}</td>
                  <td className="p-3 text-right font-bold text-[#0A3663] dark:text-blue-400">{ro.status}</td>
                </tr>
              ))}
            </tbody>
          </table>
